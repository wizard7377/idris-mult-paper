...
However, it remains a question as to whether we can construct a type of resource algebras to enrich Idris types with multiplicity polymorphism 
...
We further define $\ident Unify \phi \psi$, which takes in two formulas and returns a type, and is defined as $\fa[y]{\nat}{\fn{y \in \phi}{y \in \psi}}$
...
It models the ``argument'' in a given formula, and its clause of \ident Eval is $\ident Eval \ident FVar x \deq x$.
...
It is a unary constructor taking a natural number, which is essentially the ``constant'' formula. 
Its branch of \ident Eval is $\veval (\ident FLit k ) x \deq k$, that is, it ignores its argument. 
...
We introduce four constructors of \vform, all of which are quite similar, all taking in two sub-formulas as agreements to the constructors, and all model binary operations. 

They are \ident FAdd (addition), \ident FMul (multiplication), \ident FMax (joins), \ident FMin (meets). 
The first two of these are sufficient to define \ident Form as an instance of the Idris \ident Num class, with the conversion being \ident FLit , addition as \ident FAdd and multiplication as \ident FMul .
...
While this sounds complex, it is in practiced quite simple, with us defining the \ident Eval branch as $\veval (\ident FApp \phi \psi) x \deq \veval \phi (\veval \psi x)$. 
