...
\begin{hypo}
  $\phi$ and $\psi$ are both formulas, and for any $y$, $\fsolve{\phi}{y}$ and $\fsolve{\psi}{y}$ are decidable
\end{hypo}

We ultimately end up supplying these prerequisties using the result itself by induction.

We first note that there is a way to ``split'' each of these operations into two parts:

\begin{lemma}
  The existence of natural numbers $x$ and $y$, where $x \circ y = z$ such that $\fsolve{\phi}{x}$ and $\fsolve{\psi}{y}$, is equivalent to $\fsolve{\phi \circ \psi}{z}$.
  That is, $\et<1>[x]{\nat} \et<1>[y]{\nat} (\fsolve{\phi}{x} \wedge \fsolve{\psi}{y} \wedge x \circ y = z)$ is equivalent to $\fsolve{\phi \circ \psi}{z}$ 
\end{lemma}
