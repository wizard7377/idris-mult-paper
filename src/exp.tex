...
All in all, we define these as having the following types:
\begin{figure}
  \begin{align}
    \label{eq:exp_defs}
    \code{combine}_{\code{Exp}} &\colon \fn<1>[x]{\txp{\phi}{t}} \fn<1>[y]{\txp{\psi}{t}} \afa<0>{x_1 \peq y_1} \txp{\phi+\psi}{t} \\
    \code{split}_{\code{Exp}} &\colon \fa<1>[n]{\nat} \fa<1>[m]{\nat} \fn<1>{\txp{m+n}{t}} (\sg<1>{\txp{m}{t}} \txp{n}{t}) \\
    \code{expand}_{\code{Exp}} &\colon \fa<1>[n]{\nat} \fa<1>[\phi]{\vform} \fn<1>{}
  \end{align}
\end{figure}
...
\end{remark}
\begin{proof} 
  The the proofs somewhat simply follow.
  \begin{itemize}
  \item $\txp{0}{t}$ simply reduces to $\txp[w][n]{0}{t}$, which is trivially solveable due to \addref
    \item 
