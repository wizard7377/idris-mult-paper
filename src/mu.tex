...
\begin{definition}
	There are two constructors of $M$, $\mz$ \code{MZ}, and $\odot$, \code{MS}, Which have the signatures $\mz : \tm n t w$ and $\ms : \fn<1>[w]{t}{\fn<1>{\tm n t w}{\tm {(\code{Succ} n)} t w}}$ 
\end{definition}
...
For instance, if we want to construct $x : \tm 2 {\code{String}} \verb|"value"|$, we can only construct this through \ms, an we know, but nothing but the first argument of the value, that we must take as a initial argument $\verb|value|$, and as a second value of the form $\tm 1 \code{String} \verb|"value"|$, which we then repeat one more time and get another \verb|"value"| an finally we match $\tm 0 {\code{String}} \verb|"value"|$ and get that we must have $\mz$. 

We can then say that we know that the only constructors of $\tm 2 {\code{String}} \verb|"String"|$ is $\verb|"value"| \ms \verb|"value"| \ms \mz$. 
Note the similarity between the natural number and the constructors. 
...
One important fact about \musym{} is we can always ``lower'' a unrestricted value into \musym{}.
That is, there is a function $\code{gen} : \fn<\omega>[w]{t} \tm{n}{t}{w}$.
Unfourtanetly, because Idris dosen't have a way to ``thread'' together infinite linear values to get an unrestricted one, the inverse of this does not exist.
...
It has a signature $\code{once}: \fn<1>{\tm 1 t w}{t}$, where we have $\code{once}(x \ms \mz) \deq x$
...
\begin{proof}
	...
	The second one splits has $x : \tm {(S m')} t w$, and we can destruct on the only possible constructor of $M$ for $S$, $\ms$, and get $y :_1 t$ and $z :_1 M n t w$ where $x \peq (y \ms z)$. 
	We know by the induction hypothesis that $z$ contains exactly $n'$ uses of $\ms$, and we therefore know that the constructor occurs one more times than that, or $S n'$
\end{proof}
...
This is very important for proofs on \musym.
...
One important derivation of this is that if $\musym n t w$ is well formed, we can \emph{always} construct an inhabitant of it.

\begin{lemma}[Examples of Mu]
  \label{lem:example_mu}
  There is a erased function, $\code{Example} : \fa<0>[n]{\nat} \fa<0>[t]{\tpe} \afa<0>[w]{t} \tm{n}{t}{w}$
\end{lemma}
\begin{proof}
  This is a trivial corrallary of \ref{thm:unique}
\end{proof}
...
It instead does this similarly to how others have embedded QTT in Agda \cite{lin_dyn_mult_dep,dep_mult_dep_lin}. 
...
The fact that these are equivalent becomes even more clear when we create \mcombine{}, $\code{split}$, and $\code{join}$ and $\code{expand}$, which are included in the definition of context concatenation and flattening in Granule \cite{dep_mult_dep_lin}.


\subsection{Operations on Mu}


There are a number of operations that are very important on $M$.
The first of these that we will discuss is combination.
We define this as $\mcombine : \fa<0>[m]{\nat}{\fa<0>{n}{\nat}{\fn<1>{\tm m t w}{\fn<1>{\tm n t w}{\tm {(m + n)} t w}}}}$\missingcode, and we define it inductively as $\ms \mcombine x \deq x$, and $(a \ms b) \mcombine x \deq a \ms (b \mcombine x)$.
Note the similarity between the natural number indices and the values, where $0 + x \deq x$ and $(S n) + x \deq S (n + x)$

\todo[cleanup]{Fix the alignment of these}
\begin{figure}
	\begin{align}
		\mz \mcombine &x &\deq x \\
		Z + 	  &x &\deq x
...
Per as a matter of fact, it isn't actually possible to construct \mcombine in Granule, as it would require a way to reason about type level equality, which isn't possible \cite{granule}.
...
We can this function, which is the inverse of \code{combine}, \code{split}, which has the signature%  $\code{split}: \fa<0>[m]{\nat}{\fa<1>[n]{\nat}{\fn<1>{\tm {(m + n)} t  }}$.
...
Of these, \code{expand} is by far the most complex: the Idris definition of \code{expand} is nearly as long as the other three combined.
Granted, most of this is just copying values and proofs that various values are equal to each other, but still, this is incredibily long

\begin{definition}
  We define $\code{expand} : \fa<1>[m]{\nat} \fa<1>[n]{\nat} \fn<1>{\tm{m * n}{t}{w}} {\tm{m}{\tm{n}{t}{w}}{?}}$, which essientally repeatidly applies $\code{split}$ for the number of outer copies, each time splitting off $n$ values, and then turning them into a $\musym$. \footnote{The Idris definition is found at $\code{Data.Grade.Mu.Lemma}$}
\end{definition}

This serves as the inverse to context flattening, and, when discussing exponential types, is very roughly equivalent to numerical division. \cite{lin_distr}

\subsection{Applications over Mu}


One of the most important operations possible operations on \musym{} is $\map$, which takes a given linear function $f : \fn<1>[x]{t} (p x)$ and maps it to a function that takes in a type of $\musym$ and returns a type on $\musym$.
This defines a ``functor'' from the category of linear functions on Idris types to the ``category'' of $\musym$ on its second and third arguement.
However, \map{} will have a couple cavaets:

\begin{itemize}
\item It will have the morphism be external to the linear category, as it will need to use them more than once.
\item It has to map over \emph{both} the type and the value inside $\musym$. 
\end{itemize}

\begin{lemma}[Mapping Mu]
  \label{lem:map_mu}
  The is a function $\map : \fn<\omega>[f]{(\fn<1>[x]{t} (p x))} \fn<1>{\tm{n}{t}{w}} \tm{n}{(p w)}{(f w)}$
\end{lemma}
\begin{proof}
  We define this by inducting over $n$.

  The first of these cases, of the form $\map : \fn<\omega>[f]{(\fn<1>[x]{t} (p x))} \fn<1>{\tm{0}{t}{w}} \tm{0}{(p w)}{(f w)}$, is trivial, as it is simply $f \mz \deq \mz$.

  The second of these cases, of the form $\map : \fn<\omega>[f]{(\fn<1>[x]{t} (p x))} \fn<1>{\tm{(S n')}{t}{w}} \tm{(S n')}{(p w)}{(f w)}$, must therefore have a left hand side that ultimately can be split into $x \ms y$, where $w : t$ linearly, and $y : \tm{n'}{t}{w}$, also linearly.
  We can simply apply $f$ to $w$ to get $f w : p w$, and then on $y$ recurse to get $\map f y : \tm{n'}{p w}{f w}$, which we call $z$.
  We then can simply combine these with $\ms$ to get $x \ms \map f y$, which has the desired type
\end{proof}

This equivalent to the fact that we can lift linear functions to graded ones.
However, one fact about this that isn't quite satisfying is that we use an $\omega$ binding (of $f$), which we are trying to avoid.
Indeed, we know exactly how many $f$ we need: we apply it exactly $n$ times, so we need exactly $n$ $f$.
It is for this reason that we construct \code{app}, which acts as the ``internal'' equivalent of \map{}.

\begin{lemma}[Applying Mu]
  We can construct $\code{app} : \fn<1>{\tm{n}{(\fn<1>{t} u)}{w_f}} \fn<1>{\tm{n}{t}{w_x}} \tm{n}{u}{(w_f w_x)}$
\end{lemma}
\begin{proof}
  This construction is essientally the same as \ref{lem:map_mu}, except now we also induct on the function, however, it is otherwise the exact same.
\end{proof}

We can actually derive $\code{map}$ from $\code{app}$ and $\code{gen}$, simply as $\map f x \deq \code{app} (\code{gen} f) x$
This is important: ultimately, we can redefine $\map$ as a simple variation on $\code{app}$ that first moves everything into $\musym$ and then reasons about it.
We could, of course, have done the same thing in reverse, first reasoning about \code{app} as \code{map}, but this duality is significant for the usage of $\musym$: we don't need to ``go outside'' this linear fragment to get function application. 
...
In Idris, we define $\code{push} : \oldfn1 \_ : \tm n {(\oldsg1 \_ : t . u)} {(\tp1 \_ : w_0 . w_1)} . \oldsg1 \_ : \tm n t w_0 . \tm n u w_1$ and $\code{pull} : \oldfn1 \_ : {\oldsg1 \_ : \tm n t w_0 . \tm n u w_1} . \tm n {(\oldsg1 \_ : t . u)} {(\tp1 \_ : w_0 . w_1)}$. \todo{Expose}


\subsection{Infinite Co-Mu}


\label{sec:inf_mu}

We also define a coinductive version of mu that works with conatural numbers (as opposed to just natural numbers).
While these ``co-mu'' types are a extension of mu types, they are much more difficult to work with.
This is because matching on them no longer involves matching on the result of a constructor \code{MS}, but rather on the result of a function.

To make these slightly easier to work with, we define functions from these \code{CMu} types and \code{Mu} types, as to allow us to re-use the same functions for \code{CMu}. \missingcode{}
