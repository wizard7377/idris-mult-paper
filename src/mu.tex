...
Notably, if we define a function $\ident genMu : \fn1 x : {!_* t} . \fa1 n : \nat . {\tm n t x.{\ident unrestricted }}$, which is simply defined as $\ident genMu (\ident MkBang \_) 0 \deq \mz$ and $\ident genMu (\ident MkBang x) (S n) \deq (x \ms (\ident genMu n (\ident MkBang x)))$, we can define \ident map as $\ident map \{n\} f x \deq \ident app (\ident genMu f n) x$ \todo{This isn't exactly correct}
