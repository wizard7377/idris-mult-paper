...
\begin{definition}
	There are two constructors of $M$, $\mz$ \code{MZ}, and $\odot$, \code{MS}, Which have the signatures $\mz : \tm n t w$ and $\ms : \fn1 w : t . \fn1 \_ : {\tm n t w} . \tm {(S n)} t w$ 
\end{definition}
...
There are two very basic functions that bear mention with respect to $M$. The first of these is \code{witness}, which is of the form $\code{witness}:_0 \fa0 w : t . \tm n t w \to t$.
...
We can this function, which is the inverse of \code{combine}, \code{split}, which has the signature $\code{split}: \fn1 n : \nat . \fn1 - : {\tm {(m + n)} t w} . {{\tm m t w} \lpair {\tm n t w}}$.
...
\begin{listing}
	...
	\end{minted}
	\caption{Definition of \code{map}in Idris}
	\label{lst:def_map}
\end{listing}
...
So, we define another function, \code{app}, which has the type $\code{app}: \fn1 f : {\tm n {(\fn1 \_ : t . u)} {w_f}} . \fn1 x : {\tm n t {w_x}} . {\tm n u {(w_f w_x)}}$, and a defintion given at \ref{lst:def_app}
...
\begin{listing}
	...
	\end{minted}
	\caption{Definition of \code{app}}
	\label{lst:def_app}
\end{listing}
